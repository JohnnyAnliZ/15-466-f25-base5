...
PlayMode::PlayMode(Client &client_) : client(client_), scene(*sumo_scene) {
	...
	for (auto& transform : scene.transforms) {
		...
		else if (transform.name == "Arms") arms = &transform;
		else if (transform.name == "FootAxis") foot = &transform;

	}
	if (head == nullptr) std::cout << "wdawd" << std::endl;
...
void PlayMode::update(float elapsed) {
	...
	//send the position of the torso calculated using foot and offset
	...
	for (auto const& player : game.players) {
		...
		if (&player == &game.players.front()) {
			...
			std::cout << glm::axis(rot).y << std::endl;
