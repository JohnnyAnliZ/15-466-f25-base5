...
void Player::Controls::send_controls_message(Connection *connection_) const {
	...
	auto send_button = [&](Button const &b) {
		...
		//if (b.pressed)std::cout<<"control shit are being pressed and sent" << std::endl;
...
bool Player::recv_vec3_message(Connection* connection_) {
	assert(connection_);
	auto& connection = *connection_;
	...
	auto read = [&](auto* val) {
		if (1+at+sizeof(*val) > recv_buffer.size()) {
			std::cout << recv_buffer.size() << " " << at <<" " << sizeof(*val) << std::endl;
			throw std::runtime_error("Ran out of bytes reading state message.");
		}
		std::memcpy(val, &recv_buffer[1+at], sizeof(*val));
		at += sizeof(*val);
		};
...
Player *Game::spawn_player() {
	...
	std::cout << "new playa: " << player.name << std::endl;
	return &player;
}
...
void Game::send_state_message(Connection *connection_, Player *connection_player) const {
	...
	auto send_player = [&](Player const &player) {
		...
		connection.send(player.roll);
		connection.send(player.yaw);
		connection.send(player.rightFoot);
